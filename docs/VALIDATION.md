...
## Build 32: smelting, encumbrance and pet companions

**Publication authorized September 22, 2026.** The release branch `codex/build32-smelting-pets-release` contains the completed local checkpoint plus release labels and documentation. Baseline is published Build 31 `bacfe4a5d8d548b2cc7dc74894fdcdd5f89c7c36`. This explicit instruction supersedes the earlier local-only hold. Deployment health and exact live asset hashes are verified separately after publishing.

**1,115/1,115 tests pass** with `npm test`, without failures, cancellations or skips. All **37 changed runtime JavaScript modules** pass syntax checks; **103 actual browser modules** link with valid imports/exports, including the production Three.js model loaders. `git diff --check` passes. Independent backend, interface and integration reviews have no remaining blocking finding. Implementation suite log: `/tmp/build32-pets-full-suite.log`. Release gate: `/tmp/build32-release-tests.log`.

Smelting coverage exercises universal construction, exact prepaid recipes/timing, queued output under full storage, cancellation/refunds, ruins and hammer rebuilding, active-village pause rules, ownership, invalid inputs, demolition protection, save/restart and transaction rollback. Actual steel crafting rejects raw ore, honors Tinker discounts and shop prices, survives reload, harvests four resources and deals 25 sword damage. Both ingots have real transporter routes from smelter storage to a shop, preserving weight capacity, percentage refill targets and resources through restart. Exchange/merchant coverage verifies refined prices, uncapped percentages, old saves and overflow rejection.

Encumbrance integration covers gathering, purchases, crafting, withdrawals, worker collection, player trading and recovering tools/staff above the old limit. Server movement and client prediction share the same 45% speed rule, sprint prohibition and immediate recovery after unloading. Plot, cart and worker storage remain bounded; unsafe counts remain rejected. Late-wave tests cover nights 20/21, 30/31 and 40/41, actual spawned enemy types, existing boss cadence, bounty attribution and the 120-enemy limit.

Pets now use the approved 13-species catalog. Fifty additional tests extend the prior 1,065-test checkpoint. Real purchases cover every one of the 100 rarity buckets and preserve the exact 38/30/20/10/2 distribution after completing the collection. Tests cover same-tier preference, duplicate bank refunds, overflow-pending receipts, 30-real-minute offline hatching, account privacy, shared merchant stock, retry/restart recovery and legacy schema migration. Combat integration verifies actual simulation ticks, movement/cave/wall constraints, target cancellation, swap cooldowns, bounty/assist attribution, carry bonuses, successful-hit vampire healing and nested-save failure rollback/reload. Equipped lookups reuse bounded account caches.

All **13 actual GLBs** load through production `GLTFLoader` with real decoded PNG/JPEG pixels. Tests verify source hashes, embedded resources, independent cloned skeletons, normalized weights, visible thumbnails, genuinely deforming animation clips and actual renderer bounds. They caught a static dragon export and oversized bat wings; both were corrected. Eight renderer tests cover cache reuse, independent animation, terrain, reduced motion, server-timed projectiles/cancellation, async loading, retry and disposal. All models plus their portraits are served over local HTTP with exact bytes and correct MIME types; Three.js addon imports are served with path-containment checks. The 13 models total **21,965,552 bytes**, loaded per species when needed.

The collection UI suite covers all 13 portraits/stats, equipped summaries, rarity filtering, exact visible odds, monotonic server-time incubation, authoritative hatch/refund notices, private-account responses, persistent purchase recovery, stale offers and preservation of native pointer/keyboard presses. Desktop/mobile static layout renders were inspected. Actual GLB pose contact renders show rabbit locomotion, boar strikes and bat wing/jaw attacks. Source model thumbnails were inspected; Griffin source substitution and component licenses are explicit in the bundled credits. See [pet verification renders](previews/pets/CREDITS.md) and [companion rules](PET-COMPANIONS.md).

Smelter geometry/material/clearance/disposal tests and prior static job-menu review remain valid. These model and print-layout renders are **not an interactive browser/WebGL playtest**. Chrome was obtained, but this environment rejected its required local socket with `EPERM`; no browser, sustained GPU frame-rate, perceived-sound or multiplayer difficulty result is claimed. In-game art/lighting, performance and balance still need playtesting before release. No production accounts, villages or saves were changed for these tests.
