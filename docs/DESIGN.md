...
Each season lasts 3,600 active village seconds; a full cycle takes four hours. Spring reduces wheat regrowth by 20% and tree regrowth by 10%; summer is neutral; autumn gives 25% more wheat; winter slows wheat regrowth by 25% and raises public fuel upkeep by 30%. Fractional harvest bonuses accumulate per actor and resource, including workers, and commit only on successful harvests. Empty and fallen villages do not advance the environment. Weather changes every 4–8 active minutes, weighted by season, with rain, snow, overcast and fog. Two bounded render draws, a maximum 640 particles, quality settings and cave suppression limit visual cost.

The first event starts after 10–15 active minutes; later starts are 45–75 minutes apart. Each lasts 10–15 minutes, with one active event and no repeat of the last two. A merchant caravan delivers one finite public shipment; bumper harvest and rich ore raise relevant output by 50%, capped at twice normal after seasonal bonuses; cold snaps, festivals and construction drives consume bounded public resources. Baseline upkeep consumes one timber and half a coal per active minute, carrying fractional amounts. No event spends private goods or resets prices directly. State, random schedule, completed deliveries and remaining duration persist together.
...
The Wayfarer's entrance menu exposes tavern games independently of the merchant's travel schedule. Coin flip and roulette accept 1–10,000 gold, with existing treasury reserve, payout cover and durable receipt safeguards. Texture corrections give slate overlaps and turret corner stones real clearance; directional shadows snap in light-camera axes without changing light direction or adding draw calls.
