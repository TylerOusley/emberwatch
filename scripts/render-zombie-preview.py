# Offline actual enemy geometry preview; paired with preview-zombies.mjs. Requires Python, NumPy, Pillow, and DejaVu Sans.
...
count=len(data['stats'])
mid=(count-1)/2
W,H=2400,1200
scale=560.0 if heads else float(data.get('previewScale',260.0))
...
for i in ([] if heads else range(count)):
  cx=W/2+(i-mid)*spacing*scale
  yy=H*.76
...
for tri in tris:
 ...
 if heads:
  ...
  p[:,0]-=(tri['actor']-mid)*spacing
  p[:,1]-=focus_y
  sx=p@right*scale+(tri['actor']+.5)*(W/count)
  sy=H*.48-p@up*scale
 else:
 ...
 if heads:
  x0=max(x0,int(tri['actor']*W/count+18));x1=min(x1,int((tri['actor']+1)*W/count-18))
 ...
 if data.get('lineup')=='emergence':mask&=(a*p[0,1]+b*p[1,1]+c*p[2,1]>=-.00401) # Opaque ground hides the buried portion.
...
draw.text((64,85),'Actual sculpted game models · authoritative spawn poses · studio preview lighting'+(' · Face detail' if heads else ''),font=ImageFont.truetype(f,16),fill='#acb3b7')
for i,s in enumerate(data['stats']):
 cx=((i+.5)*(W/count) if heads else W/2+(i-mid)*spacing*scale)*2/3
 draw.text((cx,673),s['kind'].upper(),font=ImageFont.truetype(bold,16),anchor='mm',fill='#e7ddcb')
