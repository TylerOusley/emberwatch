...
import { transferLimit, moveResource, PLAYER_CARRY_LIMIT } from '../shared/transfers.js';
...
test('discounted cargo expands the pack only, and moving between storage and pack conserves item counts', () => {
  ...
  moveResource({ source: cart, destination: p, resource: 'stone', action: { max: true }, capacity: PLAYER_CARRY_LIMIT });
  assert.equal(p.inventory.stone, 125); assert.equal(cart.stone, 0); assert.equal(inventoryWeight(p), 300);
  assert.equal(p.inventory.stone + cart.stone, 125);
  assert.equal(transferLimit(cart, p, 'stone', carryCapacity(p)), 0);
});

test('gathering and purchases retain weight discounts while allowing encumbrance', () => {
  ...
  ownershipAction(sim, v, p, { kind: 'gather', targetId: node.id }); assert.equal(p.durability.pickaxe, 98); assert.equal(inventoryWeight(p), 101.4);
  ...
  economyAction(sim, v, p, { kind: 'buyResource', resource: 'stone', amount: 1, maxTotal: 100 }); assert.equal(inventoryWeight(p), 100.8);
  ...
  economyAction(sim, v, p, { kind: 'merchant_buy', resource: 'coal', amount: 1 }); assert.equal(inventoryWeight(p), 100.8);
});
...
test('collecting worker cargo uses the owner utility while the worker cargo has ordinary weight', () => {
  ...
  assert.equal(p.inventory.timber, 70); assert.equal(worker.cargo.timber, 0); assert.equal(inventoryWeight(p), 112); assert.equal(inventoryWeight(worker.cargo), 0);
});
...
test('trade validation excludes kit portions and soft capacity retains bound goods', () => {
  ...
  assert.equal(p.inventory.stone, 41); assert.equal(p.inventory.food, 2); assert.equal(p.boundInventory.food, 2); assert.equal(other.inventory.food, 1);
  assert.equal(other.boundInventory?.food ?? 0, 0); assert.equal(inventoryWeight(p), 100.4);
});
