...
test('all catalog buildings have distinct self-contained native illustrations and accurate inspectable details',()=>{
  const kinds=Object.keys(BUILDING_TYPES),art=kinds.map(buildingArt);assert.equal(kinds.length,11);assert.equal(new Set(art).size,11);
...
test('one visible plan has accessible previous/next buttons, wraps through every building and remembers each plot',()=>{
  const f=fixture();assert.equal((f.html.match(/data-building-art=/g)||[]).length,1);assert.match(f.html,/aria-roledescription="carousel"/);assert.match(f.html,/aria-label="Previous building"/);assert.match(f.html,/aria-label="Next building"/);assert.match(f.html,/1 \/ 11/);assert.match(f.html,/data-shop-focus="building-construct"/);
  const visited=new Set();for(let i=0;i<11;i++){visited.add(f.carousel.getSelection('west-1'));f.buttons.find(b=>b.text==='Next building').callback();}assert.equal(visited.size,11);assert.equal(f.carousel.getSelection('west-1'),'tool_shop');
...
test('role-eligible plans come first while locked plans remain visible with their requirements',()=>{
  for(const role of ['guard','priest','villager']){const order=buildOrder(role),firstLocked=order.findIndex(id=>BUILDING_TYPES[id].role&&BUILDING_TYPES[id].role!==role);assert.equal(order.length,11);assert.ok(order.slice(firstLocked).every(id=>BUILDING_TYPES[id].role&&BUILDING_TYPES[id].role!==role));}
