...
test('accepted sales recover with consistent inventory, stock and gold after SQLite reload', async t => {
  ...
  app.store.bank(player.id, 17);
  const quote = saleQuote('timber', 95, 60);
  assert.equal(quote, 185);
  ...
  assert.equal(saved.stock.timber, 155); assert.equal(saved.players[player.id].wallet, 235);
  assert.equal(saved.players[player.id].inventory.timber, 0); assert.equal(saved.treasury, 2315);
  await app.close();
  ...
  assert.equal(recovered.stock.timber, 155); assert.equal(recovered.treasury, 2315);
  assert.equal(recovered.players[player.id].inventory.timber, 0);
  assert.equal(recovered.players[player.id].wallet, 235);
