...
test('local graphics assets reach browser clients with their exact bytes and image types',async t=>{
  ...
  const home=await(await fetch(base)).text();assert.match(home,/PLAYABLE BUILD 27/);assert.match(home,/id="lobby-graphics-button"/);assert.match(home,/id="environment-hud"/);assert.match(home,/href="\/environment.css"/);
});
