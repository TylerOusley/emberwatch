...
test('local graphics and recorded audio assets reach clients with exact bytes and MIME types',async t=>{
  ...
  for(const path of ['/assets/surfaces/grass-albedo.jpg','/assets/sky/moon-albedo.jpg',...['pickaxe-rock-1','pickaxe-rock-2','pickaxe-rock-3','pickaxe-rock-4','wood-chop'].map(name=>`/assets/audio/${name}.mp3`)]){
    const response=await fetch(base+path);assert.equal(response.status,200);assert.equal(response.headers.get('content-type'),path.endsWith('.mp3')?'audio/mpeg':'image/jpeg');assert.equal(response.headers.get('x-content-type-options'),'nosniff');
  ...
  const home=await(await fetch(base)).text();assert.match(home,/PLAYABLE BUILD 31/);assert.match(home,/id="lobby-graphics-button"/);assert.match(home,/id="environment-hud"/);assert.match(home,/href="\/environment.css"/);
});
