...
export function ensureCaves(village) {
  ...
  for (const resource of RESOURCES) {
    ...
    if (resource.caveTier === 'upper' || !['stone', 'iron', 'coal'].includes(state.type)) state.type = caveResourceType(resource.caveTier, `${village.caveSeed}:${resource.id}`, state.roll);
    state.caveVersion = 1;
  }
}
...
export function regrowCaveResource(village, resource, state) {
  ...
  state.type = caveResourceType(resource.caveTier, `${village.caveSeed}:${resource.id}`, state.roll);
