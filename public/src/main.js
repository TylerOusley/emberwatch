...
import { createPetWorld } from './pets-world.js';
...
import { carryStatus, movementSpeed } from '/shared/encumbrance.js';
...
let cameraLook=null,heldGather=null,cosmeticsWorld=null,crateEquipmentWorld=null,swordTrails=null,defenseTroopWorld=null,petWorld=null;
...
const petsUI=createPetsUI({api,getState:()=>state,getMe:()=>me,getAccountKey:()=>auth?.playerId||auth?.name||null,getActivePanel:()=>dialog.open?activePanel:null,openPanel,toast});
...
function openPanel(content,panel=null){if(activePanel)panelDetails.set(activePanel,capturePanelDetails($('panel-content')));menuRefresh.reset();chat.close();resumeMouseAfterPanel=joined&&connection.status==='connected';activePanel=panel;dialog.classList.toggle('settlement-dialog',['settlement','requests','guard-orders','progression','trading','crates','investments','tavern','tavern-stats','village-menu','graphics','academy','civic','feedback','pets'].includes(panel));keys.clear();desired.x=desired.z=0;sendInput();cameraLook?.stop();$('panel-content').innerHTML=content;restorePanelDetails($('panel-content'),panelDetails.get(panel));if(!dialog.open)dialog.showModal();}
...
function showMenu(){
 ...
 const cards={
  play:[['menu-pets','Pets & eggs','Your permanent companions',itemArt('pet_egg')],['menu-pack','Inventory','Supplies, tools and equipment',itemArt('backpack',{level:me?.backpackTier||0})],['menu-workers','Workers','Orders, deliveries and tools',itemArt('pickaxe')],['menu-atlas','Your land','Plots and the village atlas',buildingArt('mine')],['menu-orders','Troops','Commands for your barracks',buildingArt('barracks')],['menu-crates','Crates & gear','Rewards and next-run equipment','<img src="/assets/crate-items/dawnsteel_helm.png" alt="">']],
 ...
 const actions={
  'menu-pets':()=>petsUI.show(),'menu-pack':showInventory,'menu-workers':()=>settlement.show('workers'),'menu-atlas':()=>settlement.show('atlas'),'menu-orders':()=>guardOrders.show(),'menu-crates':()=>crates.show(),
...
function showBuildStatus(){
 ...
 openPanel(`<div class="village-menu"><header class="village-menu-heading"><div><p class="eyebrow">FIRST LIGHT · PLAYABLE BUILD 32</p><h2>From ore to steel.</h2><p>Smelt stronger equipment, hatch a companion and prepare for tougher later nights.</p></div><span>${buildingArt('smelter')}</span></header><div class="build-feature-grid">
 ${feature(buildingArt('smelter'),'Build a smelter','Every role can build a smelter. Load timber and iron ore, then queue iron ingots. Add coal to make steel. Finished ingots wait safely if output storage is full.')}
 ${feature(itemArt('steel_ingot'),'Supply the new metal trade','Iron shop recipes now use iron ingots. Sell ingots at the Resource Exchange, trade them with neighbors or let the steward export surplus using the current village percentage.')}
 ${feature(itemArt('pickaxe',{tier:'steel'}),'Steel equipment','Steel gathering tools collect 4 resources per basic harvest, before plot and seasonal bonuses. Steel tools last 250 uses; the steel sword deals 25 base damage.')}
 ${feature(itemArt('backpack'),'Keep the haul, feel its weight','You can gather, buy and receive items beyond your backpack allowance. While encumbered, move at 45% speed and lose sprint until you unload. Carts, buildings and worker cargo keep their storage limits.')}
 ${feature(itemArt('sword'),'Later nights change their tactics','Nights 1–20 stay the same. Later watches bring more runners, then more armored zombies, with shorter gaps between spawns. Existing bounties remain 100, 200 and 1,000 gold.')}
 ${feature(itemArt('pet_egg'),'Meet 13 permanent companions','One merchant egg hatches after 30 real minutes, including offline time. Equip a pet for extra carrying room or combat support in any village. Duplicate pets return 1,000 gold to your bank; view all five rarity odds in Pets & eggs.')}
 </div><p class="build-feature-note">Build a Smelter on your plot, or open Pets & eggs from the village menu to manage your companions.</p></div>`,'village-menu');
}

function leave(){menuRefresh.reset();panelDetails.clear();feedback.clear();petsUI.clear();petWorld?.clear();graphicsUI.clear();inventoryHUD.clear();environmentUI.clear();villageFinance.clear();defenseTroopWorld?.clear();world?.plots?.resetEffects?.();crateEquipmentWorld?.clear();crates.clear();cosmeticsWorld?.clear();swordTrails?.reset();gameAudio.reset();trading.clear();heldGather?.stop();progression.clear();requests.clear();guardOrders.clear?.();worldClock.reset();world?.resetResourceEffects?.();selected='';cameraLook.stop();settlement.clear();chat.reset();chat.setConnected(false);connection.close();joined=false;connecting=false;state=null;me=null;ownId=null;keys.clear();dialog.close();$('lobby').hidden=false;$('hud').hidden=true;$('join-button').disabled=false;for(const a of actors.values()){scene.remove(a.rig.group);a.rig.dispose();a.label?.remove();}actors.clear();poseTracks.clear();localActionId=0;lastToolUse=-Infinity;loadVillages();}
...
function updateHUD(){inventoryHUD.update(me);environmentUI.update(joined?state.environment:null);const night=state.phase==='night';$('day-label').textContent=(night?'NIGHT ':'DAY ')+state.day;$('phase-icon').textContent=night?'☾':'☀';$('phase-label').textContent=sampleSkyCycle(worldClock.sample(performance.now())?.cycle??(night?.75:.25)).label;const seconds=Math.max(0,Math.ceil(state.phaseRemaining));$('clock').textContent=String(Math.floor(seconds/60)).padStart(2,'0')+':'+String(seconds%60).padStart(2,'0');
 ...
 $('carry-status').textContent=`PACK ${inventoryWeight(me)} / ${carryCapacity(me)}`+(carryStatus(me).encumbered?' · ENCUMBERED · 45% MOVEMENT · NO SPRINT':'')+(me.carryingId?' · CARRYING A DWARF · E / G TO PUT DOWN':me.mountedHorseId?' · MOUNTED · E / G TO DISMOUNT':me.bedPlotId?' · RECOVERING IN BED · G TO LEAVE':me.carriedBy?' · AN ALLY IS CARRYING YOU':'');
 updateHotbar();
...
async function boot(){
 ...
 function frame(){heldGather?.tick();const rawDt=clock.getDelta(),dt=Math.min(rawDt,.05);elapsed+=rawDt;requestAnimationFrame(frame);menuActors.forEach((r,i)=>{r.group.visible=!joined;r.update(dt,elapsed,{moving:false,tool:i<2?'sword':i===3?'heal':'axe'});});
   ...
   petWorld.update(joined?state?.petActors||[]:[],skyTime?.time??elapsed,dt,{position:joined?predicted:null,reducedMotion:reducedMotionQuery.matches});
