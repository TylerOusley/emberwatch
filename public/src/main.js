...
let localActionId=0, lastToolUse=-Infinity, interaction=null, activePanel=null, dynamicSolids=[];
...
function openPanel(content,panel=null){chat.close();activePanel=panel;dialog.classList.toggle('settlement-dialog',['settlement','requests','guard-orders','progression','trading','crates','investments','tavern','village-menu','graphics'].includes(panel));keys.clear();desired.x=desired.z=0;sendInput();cameraLook?.stop();$('panel-content').innerHTML=content;if(!dialog.open)dialog.showModal();}
...
function showBuildStatus(){
 ...
 openPanel(`<div class="village-menu"><header class="village-menu-heading"><div><p class="eyebrow">FIRST LIGHT · BUILD 18</p><h2>A world with more life.</h2><p>Weathered materials, natural terrain and a new light over Emberwatch.</p></div><span>${buildingArt('tree_farm')}</span></header><div class="build-feature-grid">
...
window.addEventListener('keydown',e=>{if(dialog.open||chat.isFocused()||['INPUT','SELECT','TEXTAREA'].includes(e.target.tagName)||e.target.isContentEditable)return;if(!joined)return;if(e.code==='Enter'||e.code==='NumpadEnter'){e.preventDefault();if(!e.repeat)chat.focus();return;}if(e.code==='KeyT'){e.preventDefault();if(!e.repeat)chat.toggle();return;}if(['Space','Tab'].includes(e.code))e.preventDefault();keys.add(e.code);calculateInput();sendInput();if(e.repeat)return;if(/^Digit[1-8]$/.test(e.code))selectTool(hotbar[Number(e.code.at(-1))-1]);if(e.code==='KeyR')guardOrders.show();if(e.code==='KeyI')showInventory();if(e.code==='KeyM')settlement.show('atlas');if(e.code==='KeyG'){if(me?.carryingId)action('dropPlayer');else if(me?.mountedHorseId)action('dismountHorse');else if(me?.bedPlotId)action('churchLeave');}if(e.code==='KeyE')interact();if(e.code==='KeyH')showHelp();if(e.code==='Escape')showMenu();if(e.code==='KeyF'){if(document.fullscreenElement)document.exitFullscreen();else document.documentElement.requestFullscreen().catch(()=>{});}});
...
dialog.addEventListener('close',()=>{activePanel=null;heldGather.stop();$('world').focus({preventScroll:true});});
...
async function boot(){
 ...
 function frame(){heldGather?.tick();const rawDt=clock.getDelta(),dt=Math.min(rawDt,.05);elapsed+=rawDt;requestAnimationFrame(frame);menuActors.forEach((r,i)=>{r.group.visible=!joined;r.update(dt,elapsed,{moving:false,tool:i<2?'sword':i===3?'heal':'axe'});});
   ...
   if(joined&&me){
     ...
     transportWorld.update(state,dt,new Map(state.players.filter(p=>p.online&&p.mountedHorseId).map(p=>{const g=actors.get(p.id)?.rig.group;return [p.id,g?{x:g.position.x,z:g.position.z,yaw:g.rotation.y}:{x:p.x,z:p.z,yaw:p.yaw}];})));
