...
function showBuildStatus(){
 ...
 openPanel(`<div class="village-menu"><header class="village-menu-heading"><div><p class="eyebrow">FIRST LIGHT · BUILD 19</p><h2>A smoother first watch.</h2><p>Continuous gathering, reliable menu controls and a visual path through your first day.</p></div><span>${buildingArt('tree_farm')}</span></header><div class="build-feature-grid">
...
window.addEventListener('keydown',e=>{const typing=['INPUT','SELECT','TEXTAREA'].includes(e.target.tagName)||e.target.isContentEditable;if(e.code==='KeyF'&&!typing&&!e.repeat){e.preventDefault();if(document.fullscreenElement)document.exitFullscreen();else document.documentElement.requestFullscreen().catch(()=>{});return;}if(dialog.open||chat.isFocused()||typing)return;if(!joined)return;if(e.code==='Enter'||e.code==='NumpadEnter'){e.preventDefault();if(!e.repeat)chat.focus();return;}if(e.code==='KeyT'){e.preventDefault();if(!e.repeat)chat.toggle();return;}if(['Space','Tab'].includes(e.code))e.preventDefault();keys.add(e.code);calculateInput();sendInput();if(e.repeat)return;if(/^Digit[1-8]$/.test(e.code))selectTool(hotbar[Number(e.code.at(-1))-1]);if(e.code==='KeyR')guardOrders.show();if(e.code==='KeyI')showInventory();if(e.code==='KeyM')settlement.show('atlas');if(e.code==='KeyG'){if(me?.carryingId)action('dropPlayer');else if(me?.mountedHorseId)action('dismountHorse');else if(me?.bedPlotId)action('churchLeave');}if(e.code==='KeyE')interact();if(e.code==='KeyH')showHelp();if(e.code==='Escape')showMenu();});
