...
function updateHotbar(){
 ...
 $('tool-caption').textContent=itemName(selected).toUpperCase()+' · '+(!selected?'Buy your first tool at Oak & Iron':foodIds.includes(selected)?'Eat when you need it':selected==='staff'?`${pretty(me?.mana)} / ${pretty(me?.manaMax)} mana · ${me?.staffElement||'fire'} · skills in Academy menu`:selected==='musket'?`${pretty(me?.inventory?.musket_ammo)} shot · ${MUSKET.cooldown}s reload · aim toward an enemy`:selected==='bow'?'Aim toward an enemy · uses arrows':toolDescriptions[selected]||'');
}
...
function showBuildStatus(){
 ...
 openPanel(`<div class="village-menu"><header class="village-menu-heading"><div><p class="eyebrow">FIRST LIGHT · BUILD 27</p><h2>A village worth building together.</h2><p>New callings, communal defenses, deeper mines and a full tavern games collection.</p></div><span>${buildingArt('arcane_academy')}</span></header><div class="build-feature-grid">
