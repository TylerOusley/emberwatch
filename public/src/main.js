...
import { createPanelRefreshGuard, capturePanelDetails, restorePanelDetails } from './panel-refresh.js';
...
import { ROLE_DESCRIPTIONS } from '/shared/roles.js';
import { movePlayer, resetJump } from '/shared/movement.js';
import { createSkillsUI } from './skills-ui.js';
import { createCivicUI } from './civic-ui.js';
import { createCivicWorld } from './civic-world.js';
import { createMagicWorld } from './magic-world.js';
import { staffCastState } from './staff-controls.js';
...
try{const saved=JSON.parse(localStorage.getItem('emberwatch-hotbar')||'null');if(Array.isArray(saved)&&saved.length===8&&saved.every(id=>['sword','axe','pickaxe','scythe','hammer','bow','musket','staff','food','good_food','best_food','heal'].includes(id))){hotbar=saved;savedHotbar=true;}}catch{}
...
const menuRefresh=createPanelRefreshGuard({root:dialog}), panelDetails=new Map();
...
const settlement=createSettlementUI({getState:()=>state,getMe:()=>me,getActivePanel:()=>activePanel,openPanel,send,toast,showDeliveries:id=>requests.showDestination(id),showRequests:()=>requests.show(),showInvestments:()=>villageFinance.showInvestments(),showTavern:()=>villageFinance.showTavern(),showAcademy:id=>skills.show(id),showCivic:()=>civic.show(),getHotbar:()=>hotbar,setHotbar:(slot,id)=>{hotbar[slot]=id;savedHotbar=true;try{localStorage.setItem('emberwatch-hotbar',JSON.stringify(hotbar));}catch{}renderHotbar();}});
...
const feedback=createFeedbackUI({api,getState:()=>state,getMe:()=>me,getAccountKey:()=>auth?.playerId||auth?.name||null,getActivePanel:()=>dialog.open?activePanel:null,openPanel,toast});
const skills=createSkillsUI({getState:()=>state,getMe:()=>me,getActivePanel:()=>dialog.open?activePanel:null,openPanel,send,markWaypoint:point=>settlement.setWaypoint(point)});
const civic=createCivicUI({getState:()=>state,getMe:()=>me,getActivePanel:()=>dialog.open?activePanel:null,openPanel,send,markWaypoint:point=>settlement.setWaypoint(point)});
...
for(const id of ['manager','tinker','wizard'])roles[id]={name:id[0].toUpperCase()+id.slice(1),description:ROLE_DESCRIPTIONS[id],icon:id==='wizard'?'priest':'villager'};
const toolDescriptions={staff:'Cast your attuned element · Uses mana',sword:'Sweep enemies in front of you',axe:'Gather timber',pickaxe:'Gather stone',scythe:'Harvest wheat',hammer:'Restore the gate',food:'Restore your hunger',heal:'Heal or revive an ally'};
...
const itemName=id=>!id?'Empty hands':foodIds.includes(id)?({food:'Bread',good_food:'Hearty meal',best_food:'Feast'}[id]):id==='heal'?'Priest blessing':id==='staff'?'Arcane staff':id==='musket'?'Musket':`${TOOL_TIERS[me?.tiers?.[id]||'wood']?.name||'Wooden'} ${id}`;
...
function updateHotbar(){
 for(const b of document.querySelectorAll('[data-tool]')){const id=b.dataset.tool,tier=TOOL_TIERS[me?.tiers?.[id]||'wood'],bar=b.querySelector('.durability i'),durability=me?.durability?.[id]??(me?0:100);b.classList.toggle('active',id===selected);b.classList.toggle('disabled',!canEquip(me,id));b.setAttribute('aria-disabled',String(!canEquip(me,id)));b.classList.toggle('empty',me&&(foodIds.includes(id)?!me.inventory?.[id]:id==='heal'?false:id==='staff'?!canEquip(me,id):!durability));if(bar)bar.style.width=Math.min(100,Math.max(0,durability/(me?.maxDurability?.[id]||tier?.durability||100)*100))+'%';const count=b.querySelector('.slot-count');if(count)count.textContent=foodIds.includes(id)?pretty(me?.inventory?.[id]):'';const badge=b.querySelector('.slot-tier');if(badge)badge.textContent=!canEquip(me,id)?'':me?.tiers?.[id]==='iron'?'III':me?.tiers?.[id]==='stone'?'II':'I';b.title=itemName(id)+(canEquip(me,id)?'':' · Not in your inventory');}
 $('tool-caption').textContent=itemName(selected).toUpperCase()+' · '+(!selected?'Buy your first tool at Oak & Iron':foodIds.includes(selected)?'Eat when you need it':selected==='staff'?`${pretty(me?.mana)} / ${pretty(me?.manaMax)} mana · ${staffCastState(me).spell.name} costs ${staffCastState(me).spell.mana} · ${staffCastState(me).spell.cooldown}s between casts · Unbreakable`:selected==='musket'?`${pretty(me?.inventory?.musket_ammo)} shot · ${MUSKET.cooldown}s reload · aim toward an enemy`:selected==='bow'?'Aim toward an enemy · uses arrows':toolDescriptions[selected]||'');
}
function selectTool(id){heldGather?.stop();if(!canEquip(me,id)){toast(id==='heal'?'Only priests can use a blessing.':id==='staff'?me?.role==='wizard'?'Recover your staff for free at an Arcane Academy.':'Only wizards can use a staff.':'You do not have this item. Visit Oak & Iron for wooden tools.');return;}selected=id;updateHotbar();sendInput();}
renderHotbar();
...
function sendInput(){if(joined)send({type:'input',x:desired.x,z:desired.z,yaw:desired.yaw,sprint:keys.has('ShiftLeft')||keys.has('ShiftRight'),jump:keys.has('Space')&&!inputBlocked(),tool:foodIds.includes(selected)?'food':selected});}
...
function useTool({targetId=null,quiet=false}={}){
 ...
 const staff=selected==='staff'?staffCastState(me,state?.clock??0):null;
 const now=performance.now();if(now-lastToolUse<(staff?staff.spell.cooldown*1000:selected==='musket'?MUSKET.cooldown*1000:580))return;
 if(staff&&!staff.ready){toast(staff.recovery>0?'Your staff is still recovering.':`This spell needs ${staff.spell.mana} mana. You can cast again as soon as you have enough.`);return;}
 ...
 if(selected==='sword'||selected==='bow'||selected==='musket'||selected==='staff'){if(selected==='bow'||selected==='musket'||selected==='staff')desired.yaw=cameraYaw+Math.PI;if(selected==='musket'&&!(me.inventory?.musket_ammo>0)){toast('You need musket shot. Visit a stocked tinker shop.');return;}swing();action('attack');sound(selected==='musket'?'musket':'swing');}
...
function updateInteraction(){
 ...
 if(!interaction){const carried=me.carryingId;const downed=!carried&&state.players.filter(p=>p.online&&p.downed&&!p.carriedBy&&!p.bedPlotId&&!p.rescueCartId&&p.id!==ownId&&distance(me,p)<=2.5).sort((a,b)=>distance(me,a)-distance(me,b))[0];
...
function openPanel(content,panel=null){if(activePanel)panelDetails.set(activePanel,capturePanelDetails($('panel-content')));menuRefresh.reset();chat.close();resumeMouseAfterPanel=joined&&connection.status==='connected';activePanel=panel;dialog.classList.toggle('settlement-dialog',['settlement','requests','guard-orders','progression','trading','crates','investments','tavern','tavern-stats','village-menu','graphics','academy','civic','feedback'].includes(panel));keys.clear();desired.x=desired.z=0;sendInput();cameraLook?.stop();$('panel-content').innerHTML=content;restorePanelDetails($('panel-content'),panelDetails.get(panel));if(!dialog.open)dialog.showModal();}
...
let villageMenuTab='play';
function showMenu(){
 if(!joined)return;
 const tabs=[['play','Your dwarf'],['village','Village'],['settings','Settings & help']];
 const cards={
  play:[['menu-pack','Inventory','Supplies, tools and equipment',itemArt('backpack',{level:me?.backpackTier||0})],['menu-workers','Workers','Orders, deliveries and tools',itemArt('pickaxe')],['menu-atlas','Your land','Plots and the village atlas',buildingArt('mine')],['menu-orders','Troops','Commands for your barracks',buildingArt('barracks')],['menu-crates','Crates & gear','Rewards and next-run equipment','<img src="/assets/crate-items/dawnsteel_helm.png" alt="">']],
  village:[['menu-civic','Village projects','Build shared defenses together',buildingArt('barracks')],['menu-academy','Role skills','Lessons at the Arcane Academy',buildingArt('arcane_academy')],['menu-trading','Player trading','Trade with a nearby resident',itemArt('timber')],['menu-investments','Investments','Savings, earnings and withdrawals',itemArt('gold')],['menu-tavern','The Wayfarer','Six tavern games',buildingArt('house')],['menu-tavern-stats','Betting stats','Your wins, losses and net gold',itemArt('gold')]],
  settings:[['menu-feedback','Bugs & ideas','Send a report or suggest an update',itemArt('hammer')],['menu-graphics','Graphics','Quality and performance',buildingArt('house')],['menu-help','Controls','Movement, gathering and combat',itemArt('sword')],['menu-guide','First-watch guide','Optional help for your next step',itemArt('wheat')],['menu-honors','Appearance & honors','Your milestones and earned styles','<img src="/assets/crate-items/sunforged_viking_helm.png" alt="">'],['menu-role','Change role','Choose your village calling',buildingArt('arcane_academy')]]
 };
 openPanel(`<div class="village-menu village-menu-compact"><header class="village-menu-heading"><div><p class="eyebrow">EMBERWATCH</p><h2>Village menu</h2><p>${state?.players?.filter(p=>p.online).length||0} online · Day ${state?.day||1} · ${pretty(me?.wallet)} gold</p></div></header>
 <nav class="village-menu-nav" aria-label="Menu category">${tabs.map(([id,title])=>`<button id="menu-tab-${id}" aria-pressed="${villageMenuTab===id}">${title}</button>`).join('')}</nav>
 <div class="village-menu-grid">${cards[villageMenuTab].map(card=>villageMenuCard(...card)).join('')}</div>
 ${state?.emberWardStatus?.equipped&&villageMenuTab==='play'?'<div class="panel-row"><span>Ember Ward · Nearby allies · Q</span><button id="activate-ward" '+(state.emberWardStatus.available?'':'disabled')+'>Activate</button></div>':''}
 ${me?.testAdmin&&villageMenuTab==='settings'?'<details data-persist="admin-testing"><summary>Admin testing</summary><p>Top up wallet and bank to 10,000,000 gold each.</p><button id="admin-refill-gold">Refill test gold</button></details>':''}
 ${state?.devTools&&villageMenuTab==='settings'?'<button id="start-night">Start night</button>':''}
 ${villageMenuTab==='settings'?'<p class="menu-credits"><a href="/assets/audio/CREDITS.html" target="_blank" rel="noopener">Sound credits</a></p>':''}
 <div class="village-menu-footer"><button id="menu-requests">Request board</button><button id="sound-toggle">Sound ${muted?'off':'on'}</button><button id="build-status">What’s new</button><button id="leave-button">Leave village</button></div></div>`,'village-menu');
 for(const [id]of tabs)$('menu-tab-'+id).onclick=()=>{villageMenuTab=id;showMenu();};
 const actions={
  'menu-pack':showInventory,'menu-workers':()=>settlement.show('workers'),'menu-atlas':()=>settlement.show('atlas'),'menu-orders':()=>guardOrders.show(),'menu-crates':()=>crates.show(),
  'menu-civic':()=>civic.show(),'menu-academy':()=>skills.show(),'menu-trading':()=>trading.show(),'menu-investments':()=>villageFinance.showInvestments(),'menu-tavern':()=>villageFinance.showTavern(),'menu-tavern-stats':()=>villageFinance.showTavernStats(),
  'menu-feedback':()=>feedback.show(),'menu-graphics':()=>graphicsUI.show(),'menu-help':showHelp,'menu-guide':()=>{progression.showGuide();dialog.close();},'menu-honors':()=>progression.show(),'menu-role':()=>settlement.show('roles'),
  'sound-toggle':()=>{muted=!muted;gameAudio.setMuted(muted);showMenu();},'build-status':showBuildStatus,'menu-requests':()=>requests.findBoard(),'leave-button':leave,'activate-ward':()=>action('ember_ward')
 };
 for(const [id,handler]of Object.entries(actions))if($(id))$(id).onclick=handler;
...
function showBuildStatus(){
 ...
 openPanel(`<div class="village-menu"><header class="village-menu-heading"><div><p class="eyebrow">FIRST LIGHT · BUILD 31</p><h2>Less clutter. More teamwork.</h2><p>Quicker transfers, reliable menus, shared deliveries and stronger late-game income.</p></div><span>${buildingArt('arcane_academy')}</span></header><div class="build-feature-grid">
 ${feature(itemArt('backpack'),'Quick inventory transfers','Use Store or Take directly on each item. Choose 1, 10 or Max, with exact amounts available when you need them. Your pack and building controls now keep the common actions in view.')}
 ${feature(itemArt('hammer'),'Menus that keep your click','Smaller menu categories, optional details and stable controls make busy villages easier to manage. Season, weather and events sit beside the clock; the guide stays compact until opened.')}
 ${feature(itemArt('gold'),'Uncapped village survival grants','The village earns 1,000 gold × the night survived at dawn: night 10 pays 10,000, night 100 pays 100,000. The old 5,000-gold cap is removed.')}
 ${feature(itemArt('pickaxe'),'Mine everything, share deliveries','Choose All mine resources on an owned mine to work the next available ore. Workers can deliver donations into another resident’s building; collecting from their storage is not allowed.')}
 ${feature(itemArt('hammer'),'Bugs & ideas','Open Settings & help → Bugs & ideas to send a report or suggestion and check its review status. Reports are saved for the next update review.')}
 ${feature(itemArt('axe'),'Recorded gathering sounds','Mining and woodcutting now use the approved real tool recordings, with varied pickaxe strikes and balanced volume. Sound credits are in Settings & help.')}
 ${feature(itemArt('gold'),'Bigger threats, bigger rewards',`Ordinary zombies pay ${ZOMBIE_BOUNTIES.small} gold, armored and big zombies ${ZOMBIE_BOUNTIES.large}, and Gravebreaker bosses ${ZOMBIE_BOUNTIES.boss.toLocaleString()}. Every qualifying player gets the full reward for a kill or assist, including owned towers and guards. No nightly cap.`)}
 </div><details class="build-previous" data-persist="previous-updates"><summary>Earlier updates</summary><div class="build-feature-grid">
 ...
 ${feature(itemArt('staff'),'Unbreakable Wizard staffs','Staffs no longer wear out. Existing broken staffs are restored, and Wizards can reclaim a lost staff for free at an Arcane Academy. Fire and frost cost 15 mana with a 0.6-second cast delay; lightning costs 22 with a 0.8-second delay. Cast again whenever you have enough mana.') }
 ${feature(buildingArt('house'),'Four new tavern games, together','Blackjack, three-card poker, Enchanted reels and Wheel of Fate join coin flip and roulette. Play with wallet gold at the Wayfarer entrance, day or night. Stakes reach 10,000 gold; poker reserves room for its matching play wager.')}
 ${feature(buildingArt('mine'),'A mountain with depth','The mine mouth is recessed into broad mountain foothills. Uneven stone vaults, folded walls and side workings break up the tunnels and chambers. Yellow sulfur veins remain in the deep mine. The fixed torch pool preserves stable lighting as you cross the entrance.')}
 ${feature(itemArt('pickaxe'),'Manager & equipped crews','Managers lead eight personal workers, train up to ten, and pay one gold per sixty seconds worked. Buy stone or iron worker tools for 25% or 50% more output. Optional automatic replacements use your bank. Funded crews still work while any resident is online.')}
 ${feature(buildingArt('tinker_shop'),'Tinker workshops','Tinkers can own weapon shops and reduce crafting materials for every customer in their shops, even while away. Skills improve savings from 10% to 20%. Owner-set sale prices remain independent.')}
 ${feature(buildingArt('arcane_academy'),'Wizard & six role skill trees','Wizards start with a fire staff and build Arcane Academies. Every role can pay for lasting lessons in its own skill tree. Learn frost or chain lightning, improve logistics, healing, carrying or defense. Lessons stay throughout this village run.')}
 ${feature(buildingArt('wizard_tower'),'Fire and lightning defenses','Wizard towers spend one sulfur per fireball. Upgrade to lightning that chains to at most three nearby zombies with diminishing damage. Guard skill training adds barracks troop slots.')}
 ${feature(buildingArt('barracks'),'Shared village projects','Use Village projects in the menu or Treasury to fund gate and keep reinforcement, a dedicated repair mason, a gatehouse ballista and a trebuchet. Donate bulk freight, gold and ammunition; everyone benefits.')}
 ${feature(itemArt('cart'),'Carriages built for freight and rescue','Standard storage is 1,000, upgradeable to 2,000. Loaded carts travel faster on roads, transfer directly at owned stores, and deliver project supplies. Two stretcher places carry downed friends to church beds.')}
 ${feature(itemArt('hammer'),'Rebuild with your hammer','Anyone can rebuild a destroyed structure. Hammer swings consume timber and stone; services reopen at 35% structural health. Ownership, upgrades, goods and paid troop training remain intact.')}
 ${feature(itemArt('backpack'),'Jump and explore','Press Space to jump over fallen logs and climb the stone ledges beyond the village. Tall walls and building boundaries remain solid.')}
 ${feature(itemArt('gold'),'More crate rewards & one Godly relic','Basic crates have a 5% Rare chance; Rare 10% Epic; Epic 20% Legendary. Only a Legendary crate can grant the Godly Heart of Emberwatch, at 1%. Its once-per-night Ember Ward shields nearby living allies. Press Q while equipped.')}
 ${feature(buildingArt('wheat_farm'),'Seasons, weather & village events remain active','Spring, summer, autumn and winter each last one hour of active village time, with seasonal weather and production events. The environment panel shows the current effects and countdown.')}
 </div></details><p class="build-feature-note">Your bank and permanent crate unlocks survive between villages. Role lessons and shared projects belong to this village run.</p></div>`,'village-menu');
}

function leave(){menuRefresh.reset();panelDetails.clear();feedback.clear();graphicsUI.clear();inventoryHUD.clear();environmentUI.clear();villageFinance.clear();defenseTroopWorld?.clear();world?.plots?.resetEffects?.();crateEquipmentWorld?.clear();crates.clear();cosmeticsWorld?.clear();swordTrails?.reset();gameAudio.reset();trading.clear();heldGather?.stop();progression.clear();requests.clear();guardOrders.clear?.();worldClock.reset();world?.resetResourceEffects?.();selected='';cameraLook.stop();settlement.clear();chat.reset();chat.setConnected(false);connection.close();joined=false;connecting=false;state=null;me=null;ownId=null;keys.clear();dialog.close();$('lobby').hidden=false;$('hud').hidden=true;$('join-button').disabled=false;for(const a of actors.values()){scene.remove(a.rig.group);a.rig.dispose();a.label?.remove();}actors.clear();poseTracks.clear();localActionId=0;lastToolUse=-Infinity;loadVillages();}
...
function updateHUD(){inventoryHUD.update(me);environmentUI.update(joined?state.environment:null);const night=state.phase==='night';$('day-label').textContent=(night?'NIGHT ':'DAY ')+state.day;$('phase-icon').textContent=night?'☾':'☀';$('phase-label').textContent=sampleSkyCycle(worldClock.sample(performance.now())?.cycle??(night?.75:.25)).label;const seconds=Math.max(0,Math.ceil(state.phaseRemaining));$('clock').textContent=String(Math.floor(seconds/60)).padStart(2,'0')+':'+String(seconds%60).padStart(2,'0');
 $('watch-alert').hidden=!night&&state.day%5!==0;
...
window.addEventListener('keydown',e=>{const typing=['INPUT','SELECT','TEXTAREA'].includes(e.target.tagName)||e.target.isContentEditable;if(e.code==='KeyF'&&!typing&&!e.repeat){e.preventDefault();if(document.fullscreenElement)document.exitFullscreen();else document.documentElement.requestFullscreen().catch(()=>{});return;}if(dialog.open&&e.code==='KeyE'&&!typing&&!e.repeat&&settlement.getCurrent()?.kind==='cart'){e.preventDefault();dialog.close();activePanel=null;$('world').focus({preventScroll:true});return;}if(dialog.open||chat.isFocused()||typing)return;if(!joined)return;if(e.code==='Enter'||e.code==='NumpadEnter'){e.preventDefault();if(!e.repeat)chat.focus();return;}if(e.code==='KeyT'){e.preventDefault();if(!e.repeat)chat.toggle();return;}if(['Space','Tab'].includes(e.code))e.preventDefault();keys.add(e.code);calculateInput();sendInput();if(e.repeat)return;if(/^Digit[1-8]$/.test(e.code))selectTool(hotbar[Number(e.code.at(-1))-1]);if(e.code==='KeyQ')action('ember_ward');if(e.code==='KeyR')guardOrders.show();if(e.code==='KeyI')showInventory();if(e.code==='KeyM')settlement.show('atlas');if(e.code==='KeyG'){if(me?.carryingId)action('dropPlayer');else if(me?.mountedHorseId)action('dismountHorse');else if(me?.bedPlotId)action('churchLeave');}if(e.code==='KeyE')interact();if(e.code==='KeyH')showHelp();if(e.code==='Escape')showMenu();});
...
async function boot(){
 ...
 const civicWorld=createCivicWorld(scene),magicWorld=createMagicWorld(scene);
 ...
 function frame(){heldGather?.tick();const rawDt=clock.getDelta(),dt=Math.min(rawDt,.05);elapsed+=rawDt;requestAnimationFrame(frame);menuActors.forEach((r,i)=>{r.group.visible=!joined;r.update(dt,elapsed,{moving:false,tool:i<2?'sword':i===3?'heal':'axe'});});
   ...
   if(joined&&me){
     ...
     const sprint=(keys.has('ShiftLeft')||keys.has('ShiftRight'))&&(me.hunger??100)>0;let speed=me.mountedHorseId?mountedTravelSpeed(state,me):sprint?CONFIG.sprintSpeed:CONFIG.speed;if(me.carryingId)speed=CONFIG.speed*.55;if(inventoryWeight(me)>carryCapacity(me))speed*=.65;if(Math.hypot(predicted.x-me.x,predicted.z-me.z)>3){predicted.x=me.x;predicted.z=me.z;predicted.y=me.y??groundHeight(me.x,me.z);predicted.verticalSpeed=me.verticalSpeed??0;predicted.jumpHeld=me.jumpHeld??false;}else {predicted.x=THREE.MathUtils.lerp(predicted.x,me.x,1-Math.exp(-dt*4));predicted.z=THREE.MathUtils.lerp(predicted.z,me.z,1-Math.exp(-dt*4));}if(performance.now()-lastStateAt<1500){for(const key of ['downed','mountedHorseId','bedPlotId','carriedBy','carryingId','rescueCartId'])predicted[key]=me[key];movePlayer(predicted,desired.x*speed*dt,desired.z*speed*dt,dt,keys.has('Space')&&!inputBlocked(),dynamicSolids,me.mountedHorseId?.8:CONFIG.playerRadius);}
     ...
     for(const entity of entities){activeIds.add(entity.id);let a=actors.get(entity.id);if(!a){const rig=entity.role==='zombie'?createZombiePresentation(entity,hash(entity.id)):createCharacter(entity.role||'villager',hash(entity.id));scene.add(rig.group);rig.group.position.set(entity.x,groundHeight(entity.x,entity.z),entity.z);const label=document.createElement('div');label.className='nameplate'+(entity.role==='zombie'?' enemy':'');label.innerHTML='<span></span><i><b></b></i><div class="speech-bubble" hidden></div>';$('nameplates').append(label);a={rig,label,role:entity.role,lastX:entity.x,lastZ:entity.z};actors.set(entity.id,a);}if(a.role!==entity.role){a.rig.setRole?.(entity.role);a.role=entity.role;}a.rig.setClothingColor?.(entity.color);
       ...
       group.position.y=(isMe?predicted.y:(pose?.y??entity.y))??groundHeight(group.position.x,group.position.z);if(entity.mountedHorseId||entity.carriedBy||entity.bedPlotId||entity.rescueCartId)group.position.y=groundHeight(group.position.x,group.position.z)+(entity.mountedHorseId?1.35:entity.rescueCartId?1.1:entity.carriedBy?1.25:.45);
     ...
     placeOrbitCamera({...predicted,y:predicted.y??groundHeight(predicted.x,predicted.z)},cameraYaw,cameraPitch,caveAreaAt(predicted.x,predicted.z)?Math.min(cameraDistance,4.5):cameraDistance,camDesired,camAnchor,camTarget);
   ...
   civicWorld.update(joined?state:null,dt,skyTime?.time??elapsed);magicWorld.update(joined?state:null,skyTime?.time??elapsed);
