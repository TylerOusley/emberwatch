...
export function createProgressionUI({getState,getMe,getActivePanel,openPanel,send,markWaypoint=()=>{},container=()=>document.getElementById('first-watch-guide')}) {
  ...
  function renderGuide(force=false) {
    ...
    element.innerHTML=`<div class="guide-heading"><button class="guide-toggle" aria-expanded="${expanded}" aria-controls="guide-details" aria-label="${expanded?'Collapse':'Open'} first-watch guide, ${view.completed} of ${view.total} steps complete"><span>Guide</span><strong>${view.completed}/${view.total}</strong><span aria-hidden="true">${expanded?'⌃':'⌄'}</span></button><button class="guide-dismiss" aria-label="Dismiss first-watch guide" title="Reopen from the village menu">×</button></div>${expanded?`<div id="guide-details" class="guide-details"><div class="guide-progress" aria-label="Tutorial ${progress}% complete"><i style="width:${progress}%"></i></div>${current}${checklist}</div>`:'<div id="guide-details" hidden></div>'}`;
  ...
  return {show,refresh,renderGuide,showGuide:()=>{expanded=true;command('guide_visibility',{dismissed:false});renderGuide(true);},clear(){signature='';guideSignature='';expanded=false;drafts.clear();const el=host();if(el){el.replaceChildren();el.hidden=true;}}};
}
