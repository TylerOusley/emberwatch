  ...
  return {
    update(player) {
      ...
      root.innerHTML = `<header class="pack-hud-heading"><span>ON YOU</span><button type="button" title="Open your pack (I)" aria-label="Open your full inventory">Your pack <kbd>I</kbd> ↗</button></header>
        ...
        ${model.tools.length || model.gear.length ? `<div class="pack-hud-equipment" aria-label="Carried tools and equipped items">${model.tools.map(tool => `<span title="${escape(tool.label)} · 1 carried · ${number(tool.uses)} uses remaining" aria-label="${escape(tool.label)}, 1 carried, ${number(tool.uses)} uses remaining">${itemArt(tool.id, { tier: tool.tier })}<b>1</b></span>`).join('')}${model.gear.map(item => `<span title="${escape(item.label)} · 1 equipped" aria-label="${escape(item.label)}, 1 equipped"><img src="/assets/crate-items/${item.id}.png" alt=""><b>1</b></span>`).join('')}</div>` : ''}
