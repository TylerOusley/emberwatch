...
export function academyModel(state, player, plotId, previewRole = player?.role) {
  ...
  const operational = plot?.building === 'arcane_academy' && plot.hp > 0;
  const nearby = operational && canUsePlot(player, site, plot);
  const standing = player?.online !== false && !player?.downed && !player?.bedPlotId && !player?.mountedHorseId && player?.hp > 0;
  return { plot, nearby, operational, role: previewRole, mana: Math.floor(academy.mana ?? 0), manaMax: academy.manaMax ?? 100, element: academy.staffElement ?? 'fire', elements: academy.stats?.staffElements ?? [],
...
export function createSkillsUI({ getState, getMe, getActivePanel, openPanel, send, markWaypoint = () => {}, document: doc = globalThis.document }) {
  ...
  function render() {
    ...
    const attunements = player.role === 'wizard' ? `<section class="menu-section"><div class="menu-section-content"><h3>Your staff</h3><p>${m.mana} / ${m.manaMax} mana · ${esc(title(m.element))} attunement. Mana recovers while you are alive and active.</p><div class="panel-actions">${m.elements.map(element => `<button data-academy-element="${element}" ${element === m.element ? 'disabled' : ''}>${esc(title(element))}</button>`).join('')}</div></div></section>` : '';
