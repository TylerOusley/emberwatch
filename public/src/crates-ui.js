import { CRATE_PRICES, CRATE_POOLS, CRATE_EQUIPMENT, CRATE_RULES, LOADOUT_SLOTS, GATHERING_TOOLS, emptyLoadout, normalizeLoadout, crateMilestoneTier, CRATE_UPGRADES, crateRewardOdds } from '../../shared/crates.js';
...
const image = (id, className = '') => crateItem(id) ? `<img class="${className}" src="${crateItem(id).image}" alt="${esc(crateItem(id).name)}" loading="lazy" width="400" height="360">` : '';
...
export function crateOdds(tier) { const upgrade = CRATE_UPGRADES[tier]; return upgrade ? `${100 - upgrade.percent}% ${CRATE_TIERS[tier].label} · ${upgrade.percent}% ${CRATE_TIERS[upgrade.tier].label}` : ''; }
export function crateItemOdds(tier, itemId) { const odds = crateRewardOdds(tier).find(row => row.itemId === itemId); return odds ? `${Number(odds.percent.toFixed(4))}% per opening` : ''; }
export function crateReceiptOdds(result) {
  if (result.catalogVersion === 1) {
    const count = { basic:4, rare:6, epic:4, legendary:5 }[result.tier];
    return count ? `${Number((100 / count).toFixed(4))}% at the time of opening` : '';
  }
  return crateItemOdds(result.tier, result.itemId);
}
...
export function createCratesUI({ getMe = () => null, getAccountKey, getActivePanel, openPanel, api, toast = () => {}, onAccountUpdate = () => {},
  ...
  function itemCard(id, { odds = false, sourceTier = tier } = {}) {
    ...
    return `<article class="crate-item ${owns(id) ? 'owned' : ''}" style="--crate-color:${CRATE_TIERS[item.tier].color}"><div class="crate-item-art">${image(id)}<span class="crate-item-badge">${esc(badge)}</span>${odds ? `<span class="crate-item-odds">${esc(crateItemOdds(sourceTier, id))}</span>` : ''}</div><div class="crate-item-body"><p class="crate-item-tier">${esc(CRATE_TIERS[item.tier].label)} · ${esc(slotNames[item.slot] || item.slot)}</p><h3>${esc(item.name)}</h3><p>${esc(itemDescription(item))}</p><span class="crate-owned">${id === 'phoenix_ember' ? `${pretty(data?.charges?.total)} reserve charges · repeats add +1` : equipped ? '✓ Selected for your next run' : owns(id) ? '✓ Permanently owned' : 'Not unlocked'}</span></div></article>`;
  }
  function shop() {
    const prices = CRATE_PRICES[tier], amount = prices[funding], enough = Number(data?.[funding]) >= amount;
    const tabs = Object.keys(CRATE_PRICES).map(id => button(CRATE_TIERS[id].label, () => { tier = id; render(); }, busy || Boolean(pending), `crate-tier crate-tier-card ${tier === id ? 'selected' : ''}`,
      `<span class="crate-tier-art">${chestArt(id)}${image(tierHeroes[id], 'crate-tier-treasure')}</span><span class="crate-tier-name">${esc(CRATE_TIERS[id].label)}</span><span class="crate-tier-count">${crateRewardOdds(id).length} possible rewards</span><span class="crate-tier-price">${pretty(CRATE_PRICES[id][funding])} ${funding === 'bank' ? 'gold' : 'credits'}</span>`, `aria-pressed="${tier === id}" style="--crate-color:${CRATE_TIERS[id].color}"`)).join('');
    const earned = data.earnedCrates || [];
    const rewards = crateRewardOdds(tier), ownedCount = rewards.filter(row => owns(row.itemId)).length;
  ...
  function opening() {
    ...
    const item = crateItem(result.itemId), rewardTier = result.rewardTier || item?.tier || result.tier, color = CRATE_TIERS[rewardTier]?.color || CRATE_TIERS.basic.color;
    if (spinning) {
