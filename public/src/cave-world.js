...
export function createCaveWorld(scene){
  ...
  function tint(x,y,z,floor=false){const depth=clamp(-y/14),grain=Math.sin(x*17.3+y*9.2+z*13.1)*Math.sin(x*5.3-z*7.8),band=Math.sin(y*8+x*.13+z*.21+Math.sin(x*.7-z*.2)*.4);scratch.set(floor?0xc8bda5:0xd3d7cb).lerp(new THREE.Color(0x8eaaa8),depth*.25);scratch.multiplyScalar(.94+grain*.025+(band>.72?.04:0));return scratch.toArray();}
  ...
  for(const edge of layout.walls){
    const breaks=lines([edge.a,edge.b,...(edge.vertical?CAVE_HEIGHTS.map(h=>h.z).filter(z=>z>edge.a&&z<edge.b):[])],.85),rows=[0,.26,.34,.80,.89,1.40,1.49,2.00,2.10,2.65,2.74,3.35,3.44,4.10,4.19,4.85,4.94,5.45,5.54,6.10,6.19,6.50,7.45];
    function point(t,row){const x=edge.vertical?edge.fixed:t,z=edge.vertical?t:edge.fixed,layer=Math.floor(row/2),y=groundHeight(x,z)+rows[row]+(row===0?0:(.16*Math.sin(t*.36+layer*.18)+.11*Math.sin(t*.93)+.03*Math.sin(t*1.91+layer*.8))*Math.min(1,rows[row]/.8)),bulge=row===0?.025:row===rows.length-1?0:(row%2?.035:.14)+.025*Math.sin(t*2.7+layer*5);return [x+edge.nx*bulge,y,z+edge.nz*bulge];}
