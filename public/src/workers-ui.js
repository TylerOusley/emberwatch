...
export function workerEquipmentPanel({ worker, player, command, row }) {
  ...
  let html = transporter ? '<p>Transporters move stored goods and do not use harvesting tools. Train Movement or Carrying to improve deliveries.</p>' : '<h4>Worker tools</h4><p>Buy tools directly for this worker. Stone: 30g / +25% output. Iron: 100g / +50% output. Paid from your wallet; no carried tools needed.</p>';
  ...
  for (const tool of ['axe', 'pickaxe', 'scythe']) {
    ...
    html += `<article class="worker-tool-shop${active ? ' current' : ''}"><div class="worker-tool-shop-art">${itemArt(tool, { tier: effective.tier })}</div><div class="worker-tool-shop-copy">` + row(title, supplied ? `${supplied.tier} · ${supplied.durability} / ${supplied.maxDurability} durability${effective.tier === 'wood' ? ' · using wooden fallback' : ''}` : 'Standard wooden equipment');
  ...
  if (!transporter || worker.autoReplaceEnabled) {
    html += '<h4>Auto-replacement</h4><p>Broken tools are replaced from your bank: stone 30g · iron 100g. If funds run out, wooden tools are used until you top up. Wages still use your wallet.</p>';
