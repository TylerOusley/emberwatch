...
export function createRequestsUI({ getState, getMe, getActivePanel, openPanel, closePanel = () => {}, send, markTarget, document: doc = globalThis.document }) {
  ...
  function render() {
    ...
    const html = '<div class="settlement-panel ' + (board ? 'request-board-view' : 'request-counter-view') + '"><p class="eyebrow">' + (board ? 'VILLAGE REQUEST BOARD' : 'REQUESTED DELIVERIES') + '</p><h2>' + (board ? 'Supplies for the next watch.' : esc(destination(view.id).name)) + '</h2>' +
      ...
      '<details class="menu-disclosure" data-persist="delivery-rules"><summary>How paid deliveries work</summary><p>Ordinary sales and donations still work. They can fill a shortage and close its request; use “Deliver supplies” at the requested destination to receive the posted payment. Removing stored supplies does not create a rewarded shortage.</p></details>' +
      (history.length ? '<details class="menu-disclosure" data-persist="request-history"><summary>Recent requests</summary>' + history.map(r => `<div class="settlement-row"><div><strong>${esc(title(r.resource))} · ${esc(r.destinationName)}</strong><small>${esc(title(r.status))} · ${r.delivered} delivered · ${esc(r.reason)}</small></div></div>`).join('') + '</details>' : '') + '</div>';
